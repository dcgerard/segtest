...
For autopolyploids, specify `type = "polysomic"` and, optionally, the amound of double reduction via `alpha`. `alpha` is a vector of length `floor(ploidy / 4)` where element `i` is the probability a gamete has genotype `i`. The upper bounds for `alpha` can be found via `drbounds()`. E.g., for a parental octoploid with genotype 4 with no and moderate levels of double reduction:
```{r}
...
For allopolyploids, the possible gamete frequencies can be found in `seg`. E.g., for a prental octoploid with genotype 4, the possible gamete frequencies are
```{r}
...
My recommendation is to always use the genotype log-likelihoods with known parent genotypes (typically estimated via your genotyper). But `seg_lrt()` allows for both known offspring genotypes and for unknown parental genotypes, if that situation works best for you.
...
We allow for some non-valid genotypes via the `ob` argument. This is the upperbound on the proportion of outliers. By default, this is set to 0.03. You can set this to `0` (or set `outlier = FALSE`) if you want any outliers to indicate segregation distortion.
